...
public class ShadowLayout extends FrameLayout {
    ...
    private boolean isShadowBottom = true;
    private boolean isShadowTop = true;
    private boolean isShadowStart = true;
    private boolean isShadowEnd = true;
        ...
        try {
            ...
            setShadowTop(
                    typedArray.getBoolean(
                            R.styleable.ShadowLayout_sl_is_shadow_top, isShadowBottom
                    )
            );
            setShadowBottom(
                    typedArray.getBoolean(
                            R.styleable.ShadowLayout_sl_is_shadow_bottom, isShadowBottom
                    )
            );
            setShadowEnd(
                    typedArray.getBoolean(
                            R.styleable.ShadowLayout_sl_is_shadow_end, isShadowEnd
                    )
            );

            setShadowStart(
                    typedArray.getBoolean(
                            R.styleable.ShadowLayout_sl_is_shadow_start, isShadowStart
                    )
            );
        } finally {
            typedArray.recycle();
    ...
    public boolean isShadowBottom() {
        return isShadowBottom;
    }

    public void setShadowBottom(boolean shadowBottom) {
        isShadowBottom = shadowBottom;
        resetShadow();
    }
    ...
    public void setShadowTop(boolean shadowTop) {
        isShadowTop = shadowTop;
        resetShadow();
    }
    ...
    public void setShadowEnd(boolean shadowEnd) {
        isShadowEnd = shadowEnd;
        resetShadow();
    }

    public boolean isShadowStart() {
        return isShadowStart;
    }

    public void setShadowStart(boolean shadowStart) {
        isShadowStart = shadowStart;
        resetShadow();
    }
    ...
    private void resetShadow() {
        ...
        int start = 0, end = 0, bottom = 0, top = 0;
        // Set padding for shadow bitmap
        final int padding = (int) (mShadowRadius);

       // Log.d(TAG, "resetShadow: top: " + isShadowTop);
       // Log.d(TAG, "resetShadow: bottom: " + isShadowBottom);
       // Log.d(TAG, "resetShadow: end: " + isShadowEnd);
       // Log.d(TAG, "resetShadow: start: " + isShadowStart);
        if (isShadowTop)
            top = padding;
        else if (isShadowBottom)
            bottom = padding;
        else if (isShadowStart)
            start = padding;
        else if (isShadowEnd)
            end = padding;
